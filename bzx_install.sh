...
function get_ip() {
  declare -a NODE_buzzexcoin
  for buzzexcoin in $(netstat -i | awk '!/Kernel|Iface|lo/ {print $1," "}')
  do
    NODE_buzzexcoin+=($(curl --interface $buzzexcoin --connect-timeout 2 -s4 icanhazip.com))
  done

  if [ ${#NODE_buzzexcoin[@]} -gt 1 ]
    then
      echo -e "${GREEN}More than one IP. Please type 0 to use the first IP, 1 for the second and so on...${NC}"
      INDEX=0
      for ip in "${NODE_buzzexcoin[@]}"
      do
        echo ${INDEX} $ip
        let INDEX=${INDEX}+1
      done
      read -e choose_ip
      NODEIP=${NODE_buzzexcoin[$choose_ip]}
  else
    NODEIP=${NODE_buzzexcoin[0]}
  fi
}
